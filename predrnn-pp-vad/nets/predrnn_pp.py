...
def rnn(images, mask_true, num_layers, num_hidden, filter_size, stride=1,
    ...
    for t in xrange(seq_length-1):
        ...
        with tf.variable_scope('predrnn_pp', reuse=reuse):
            ...
            print("predrnn_pp: type inputs", type(inputs))
            print("predrnn_pp: type hidden[0]", type(hidden[0]))
            print("predrnn_pp: shape inputs", inputs.shape)
            print("predrnn_pp: shape hidden[0]", hidden[0].shape)
            print("predrnn_pp: shape cell[0]", cell[0].shape)
            print("predrnn_pp: shape mem", mem.shape)
    ...
    return [gen_images, loss] #gen_images e' un tensore, quindi qui non ha ancora un valore. Verra' calcolato dopo
