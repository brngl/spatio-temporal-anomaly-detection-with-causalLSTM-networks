    ...
    def __call__(self, x, h, c, m):
        ...
        with tf.variable_scope(self.layer_name):
            ...
            if x is None:
                ...
                print("CasualLSTMCell: x.shape", x.shape)
                print("CasualLSTMCell: x_cc.shape", x_cc.shape)
                i_x, g_x, f_x, o_x, i_x_, g_x_, f_x_ = tf.split(x_cc, 7, 3)
                print("CasualLSTMCell: i_x.shape", i_x.shape)
                print("CasualLSTMCell: i_x.shape", g_x.shape)
                print("CasualLSTMCell: i_x.shape", f_x.shape)
                print("CasualLSTMCell: i_x.shape", o_x.shape)
                print("CasualLSTMCell: i_x_.shape", i_x_.shape)
                print("CasualLSTMCell: g_x_.shape", g_x_.shape)
                print("CasualLSTMCell: f_x_.shape", f_x_.shape)

                i = tf.sigmoid(i_x + i_h + i_c)
                f = tf.sigmoid(f_x + f_h + f_c + self._forget_bias)
                g = tf.tanh(g_x + g_h + g_c)
                print("CasualLSTMCell: i.shape", i.shape)
                print("CasualLSTMCell: f.shape", f.shape)
                print("CasualLSTMCell: g.shape", g.shape)
            ...
            print("CasualLSTMCell: c2m.shape", c2m.shape)
            i_c, g_c, f_c, o_c = tf.split(c2m, 4, 3)
            print("CasualLSTMCell: i_c.shape", i_c.shape)
            print("CasualLSTMCell: g_c.shape", g_c.shape)
            print("CasualLSTMCell: f_c.shape", f_c.shape)
            print("CasualLSTMCell: o_c.shape", o_c.shape)
            ...
            print("CasualLSTMCell: ii.shape", ii.shape)
            print("CasualLSTMCell: ff.shape", ff.shape)
            print("CasualLSTMCell: gg.shape", gg.shape)

            m_new = ff * tf.tanh(m_m) + ii * gg
            print("CasualLSTMCell: m_new.shape", m_new.shape)
            ...
            print("CasualLSTMCell: o_m.shape", o_m.shape)
